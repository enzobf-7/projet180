...
export function TopBar({ jourX, daysLeft, daysPct, firstName, navItems, onSignOut, onboardingDate, robinWhatsapp }: Props) {
  ...
  return (
    <header style={{
      ...
      {isMobile ? (
        <>
          {/* Mobile row 1: logo + Robin + avatar */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', height: 50 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {robinWhatsapp && (
                <a href={`https://wa.me/${robinWhatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer"
                  style={{ display: 'flex', alignItems: 'center', gap: 4, background: '#25D366', borderRadius: 20, padding: '4px 10px 4px 7px', textDecoration: 'none' }}>
                  ...
                  <span style={{ ...D, fontWeight: 700, fontSize: '9px', color: 'white', letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>Robin</span>
                </a>
              )}
              <div style={{ width: 26, height: 26, background: C.accent, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ ...D, fontWeight: 900, fontSize: '11px', color: 'white' }}>{firstName.charAt(0).toUpperCase()}</span>
              </div>
              <button onClick={onSignOut} title="Déconnexion" style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted, fontSize: '14px', lineHeight: 1, padding: 2 }}>⏻</button>
            </div>
          </div>
          {/* Mobile row 2: nav full width */}
          <nav style={{ display: 'flex', padding: '0 8px 6px', gap: 4 }}>
            {navItems.map(item => (
              <a key={item.href} href={item.href} style={{
                flex: 1, textAlign: 'center' as const,
                padding: '7px 4px',
                ...D, fontWeight: 700, fontSize: '11px', letterSpacing: '0.06em',
        ...
        /* Desktop row: logo + nav + user */
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', padding: '0 20px', height: 64 }}>
          {/* Logo */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {robinWhatsapp && (
              <a href={`https://wa.me/${robinWhatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer"
                style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#25D366', borderRadius: 20, padding: '5px 12px 5px 8px', textDecoration: 'none', transition: 'opacity 0.2s' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                <span style={{ ...D, fontWeight: 700, fontSize: '10px', color: 'white', letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>Robin</span>
              </a>
            )}
          </div>
          {/* Nav */}
          <nav style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
            {navItems.map(item => (
              <a key={item.href} href={item.href} style={{
                padding: '10px 20px', ...D, fontWeight: 700, fontSize: '15px',
                letterSpacing: '0.12em', textTransform: 'uppercase' as const, textDecoration: 'none',
                color: item.active ? 'white' : C.accent,
                background: item.active ? 'rgba(58,134,255,0.25)' : 'rgba(58,134,255,0.08)',
                borderRadius: 8, transition: 'all 0.15s',
              }}>{item.label}</a>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'flex-end' }}>
            <div style={{ width: 30, height: 30, background: C.accent, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ ...D, fontWeight: 900, fontSize: '13px', color: 'white' }}>{firstName.charAt(0).toUpperCase()}</span>
            </div>
            <button onClick={onSignOut} title="Déconnexion" style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted, fontSize: '16px', lineHeight: 1, padding: 4 }}>⏻</button>
          </div>
        </div>
      )}
